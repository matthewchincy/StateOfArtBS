...
SJN_MultiCueBGS::SJN_MultiCueBGS() : firstTime(true)
{
	//----------------------------------
...
SJN_MultiCueBGS::SJN_MultiCueBGS(int g_iTrainingPeriod2, int g_iT_ModelThreshold2, int g_iC_ModelThreshold2, float g_fLearningRate2,
	...
	: firstTime(true)
{
	...
	//	User adjustable parameters
	//----------------------------------			
	g_iTrainingPeriod = g_iTrainingPeriod2;											//the training period								(The parameter t in the paper)
	g_iT_ModelThreshold = g_iC_ModelThreshold2;							//the threshold for texture-model based BGS.		(The parameter tau_T in the paper)
	g_iC_ModelThreshold = g_iC_ModelThreshold2;										//the threshold for appearance based verification.  (The parameter tau_A in the paper)

	g_fLearningRate = g_fLearningRate2;											//the learning rate for background models.			(The parameter alpha in the paper)

	g_nTextureTrainVolRange = g_nTextureTrainVolRange2;									//the codebook size factor for texture models.		(The parameter k in the paper)
	g_nColorTrainVolRange = g_nColorTrainVolRange2;										//the codebook size factor for color models.		(The parameter eta_1 in the paper)

	g_bAbsorptionEnable = TRUE;										//If TRUE, cache-book is also modeled for ghost region removal.
	g_iAbsortionPeriod = g_iAbsortionPeriod2;										//the period to absorb static ghost regions 

	g_iRWidth = g_iRWidth2, g_iRHeight = g_iRHeight2;								//Frames are precessed after reduced in this size .

																	//------------------------------------
																	//	For codebook maintenance
																	//------------------------------------
	g_iBackClearPeriod = g_iBackClearPeriod2;										//the period to clear background models	
	g_iCacheClearPeriod = g_iCacheClearPeriod2;										//the period to clear cache-book models	

																	//------------------------------------
																	//	Initialization of other parameters
																	//------------------------------------
	g_nNeighborNum = g_nNeighborNum2, g_nRadius = g_nRadius2;
	g_fConfidenceThre = g_iT_ModelThreshold / (float)g_nNeighborNum;	//the final decision threshold

	g_iFrameCount = g_iFrameCount2;
	g_bForegroundMapEnable = FALSE;									//TRUE only when BGS is successful
	g_bModelMemAllocated = FALSE;									//To handle memory..
	g_bNonModelMemAllocated = FALSE;								//To handle memory..
}
...
// Save parameters
void SJN_MultiCueBGS::SaveParameter(std::string folderName) {
	std::ofstream myfile;
