...
PBAS::PBAS(float alpha,float beta,int N,int Raute_min ,	float R_incdec ,int R_lower ,int R_scale ,
	float T_dec ,int T_inc,int T_init ,	int T_lower,int T_upper) :
	alpha(alpha),beta(beta),N(N),R_lower(R_lower), Raute_min(Raute_min), T_lower(T_lower), T_upper(T_upper),
	R_scale(R_scale), R_incdec(R_incdec), T_dec(T_dec), T_inc(T_inc), T_init(T_init)
{
	//feature vector
	formerMeanNorm = 0;
	width = 0;
...
void PBAS::operator()(cv::InputArray _image, cv::OutputArray _fgmask)
{
	cv::Mat input = _image.getMat();
	if(width != input.cols)
	{
		width = input.cols;
	...
	_fgmask.create(input.size(), CV_8UC1);
	cv::Mat output = _fgmask.getMat();
	memset(output.data, 0, output.cols*output.rows);
	...
	for (int j=0; j< output.rows; ++j)
	{
		...
		{
			uchar& resultMap = output.data[(j*output.cols)+i];
			...
			//update T
			learningRateRegulator(actualT_Pt, meanMinDist_Pt, &resultMap);
