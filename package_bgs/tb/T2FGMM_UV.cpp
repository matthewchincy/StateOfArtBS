...
T2FGMM_UV::T2FGMM_UV() : firstTime(true), frameNumber(0), threshold(9.0), alpha(0.01), km(1.5f), kv(0.6f), gaussians(3)
{
}
T2FGMM_UV::T2FGMM_UV(long frameNumber, double threshold, double alpha, float km, float kv, int gaussians)
	: firstTime(true), frameNumber(frameNumber), threshold(threshold), alpha(alpha), km(km), kv(kv), gaussians(gaussians)
{
}
T2FGMM_UV::~T2FGMM_UV()
{
...
// Save parameters
void T2FGMM_UV::SaveParameter(std::string folderName) {
	std::ofstream myfile;
